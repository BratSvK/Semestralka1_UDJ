...
namespace structures 
{
	...
	Array<T>::~Array()
	{
		// znicit z coho vsetkeho sa to sklada 
		delete vector_;
		vector_ = nullptr;
		size_ = 0;
	}

	// vzdy kopirovaci konstruktor
	template<typename T>
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		if (this != &other)
		{
			// vlastnosti pola velkost pola je nemmenna
			// len rovnake velkosti 
			if (size_ != other.size_)
			{
				throw std::invalid_argument("Arrays sizes differ!");
			}

			// ako priradim jedno pole do druheho 
			//vector_ = other.vector_;				// pozor na toto ukazovatele na vectori, toto je zle smerniky na vectori, MEMORY LEAK !!!!
			// vectori na co smerniky ukazuju 
	...
	// toto robi pole[3] = 4
	// 4 kroky
	// 1. kde je nas int napriklad vo vectore
	// 2. dostat sa nas miesto kde zacina nase cislo
	// 3. musim skumat smernik na celu premmenu 
	// 4. ked mam 4 byte tak sa mozem pozriet do vnutra a dostanem premmenu (dereferencia)

	template<typename T>
	T& Array<T>::operator[](const int index)
	{
		// kde zacinam vo vectore 
		// mapovacia funckia -> predpis ktory mi transformuje index prvku v poli na index prvku vo vectore (0-> 0) (1-> 4) int
		// osetrenie v mapovacej funkcii
	...
	// int a = pole[3] vracia mi kopiu hodnotu 
	template<typename T>
	...
	{
		// kazdej velkosti pola zodpoveda velkost vectora 
	...
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		if (length != 0)
		{
			// pole ma v sebe vector, int = 4 byte 
